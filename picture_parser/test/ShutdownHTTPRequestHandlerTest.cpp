...
#include "picture_parser/ShutdownHTTPRequestHandler.h"
...
TEST_GROUP(ShutdownHTTPRequestHandlerTest) {
    TEST_SETUP() {
        ...
        m_uut = new PictureParser::ShutdownHTTPRequestHandler(m_shutdown);
    }
    ...
    PictureParser::ShutdownHTTPRequestHandler *m_uut;
};
...
TEST(ShutdownHTTPRequestHandlerTest, handlePostRequestShutdownTrueTest) {
    ...
    m_request->m_ss << PictureParser::ShutdownHTTPRequestHandler::SHUTDOWN_FORM_DATA << "=true";

    EXPECT_CALL(*m_shutdown, shutdown());

    m_uut->handleRequest(*m_request, *m_response);

    CHECK_EQUAL(m_response->m_response.str(), PictureParser::ShutdownHTTPRequestHandler::POST_TRUE_MESSAGE);
...
TEST(ShutdownHTTPRequestHandlerTest, handlePostRequestShutdownFalseTest) {
    ...
    // Form data from a post request
    m_request->m_ss << PictureParser::ShutdownHTTPRequestHandler::SHUTDOWN_FORM_DATA << "=false";

    m_uut->handleRequest(*m_request, *m_response);

    CHECK_EQUAL(m_response->m_response.str(), PictureParser::ShutdownHTTPRequestHandler::POST_FALSE_MESSAGE);
    CHECK_EQUAL(m_response->_status, Poco::Net::HTTPResponse::HTTP_OK);
}
...
TEST(ShutdownHTTPRequestHandlerTest, handleGetRequestTest) {
    ...
    CHECK_EQUAL(m_response->m_response.str(), PictureParser::ShutdownHTTPRequestHandler::GET_MESSAGE);
    CHECK_EQUAL(m_response->_status, Poco::Net::HTTPResponse::HTTP_OK);
}
