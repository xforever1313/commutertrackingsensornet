...
static const unsigned long	CCV_GARBAGE       = 0x80000000; // matrix is in cache (not used by any functions)
static const unsigned long CCV_REUSABLE      = 0x40000000; // matrix can be recycled
static const unsigned long CCV_UNMANAGED     = 0x20000000; // matrix is allocated by user, therefore, cannot be freed by ccv_matrix_free/ccv_matrix_free_immediately
static const unsigned long CCV_NO_DATA_ALLOC = 0x10000000; // matrix is allocated as header only, but with no data section, therefore, you have to free the data section separately
...
__inline static ccv_margin_t ccv_margin(int left, int top, int right, int bottom)
{
	ccv_margin_t margin;
	margin.left = left;
	margin.top = top;
...
__inline static ccv_size_t ccv_size(int width, int height)
{
	ccv_size_t size;
	size.width = width;
	size.height = height;
...
__inline static int ccv_size_is_zero(ccv_size_t size)
{
...
__inline static ccv_rect_t ccv_rect(int x, int y, int width, int height)
{
	ccv_rect_t rect;
	rect.x = x;
	rect.y = y;
...
__inline static int ccv_rect_is_zero(ccv_rect_t rect)
{
...
__inline static ccv_point_t ccv_point(int x, int y)
{
	ccv_point_t point;
	point.x = x;
	point.y = y;
...
__inline static ccv_decimal_point_t ccv_decimal_point(float x, float y)
{
...
__inline static ccv_decimal_pose_t ccv_decimal_pose(float x, float y, float a, float b, float roll, float pitch, float yaw)
{
...
__inline static ccv_categorized_t ccv_categorized(int c, ccv_dense_matrix_t* matrix, ccv_file_info_t* file)
{
